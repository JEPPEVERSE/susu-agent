...
`issue_type` 按失败原因选择：最终答案错误用 `final_answer_error`，推导错误用 `step_reasoning_error`，关键推导缺口用 `derivation_gap`，遗漏条件或边界用 `condition_omission`，教案引用错误用 `lesson_plan_mismatch`，现有证据不足以验证用 `evidence_insufficient`。填写实际核对的 `checked_condition_indices`；证据不足时令 `evidence_sufficient=false`。只有会影响答案正确性、推导有效性或教案步骤归属的问题才使用 `severity="error"`。只输出 VerificationReport。
